...
import 'package:egovapp/page/kehoach/xuly/kehoach-edit.dart';
...
class AppBarKeHoachXuLyAction extends StatefulWidget {
  const AppBarKeHoachXuLyAction({Key? key, required this.id,
    required this.title,
  ...
  final String title;
  @override
  _AppBarKeHoachXuLyActionState createState() => _AppBarKeHoachXuLyActionState();
}

class _AppBarKeHoachXuLyActionState extends State<AppBarKeHoachXuLyAction> {
  ...
  dynamic _delete() async
  {
    ...
    ServiceKeHoach().delete(this.widget.id).then((value){
            ...
            PopupMenuButton(
                ...
                onSelected: (String value) async{
                  if(value==ParamUtils.actionUpdate) {
                    Navigator.push(context, new MaterialPageRoute(builder: (context) => new EditKeHoach(id: this.widget.id,
                        title: Language.getText('capnhatkehoach'), callBackRefresh: () =>
                            setState((){ })
                    )),);
                  }
