...
class EditYeuCau extends StatefulWidget
{
  const EditYeuCau({Key? key,
    required this.id,
    required this.title,
  ...
  final int id;
  @override
  _EditYeuCauState createState() => _EditYeuCauState();}
class _EditYeuCauState extends State<EditYeuCau>
{
  ...
  void initState(){
    ...
    if(this.widget.id !=0) {
      ...
      ServiceYeuCau().getById(this.widget.id).then((value) {
        ...
        if (value.yeuCauFiles.length > 0) {
          for (var f in value.yeuCauFiles)
  ...
  async {
    ...
    else
    {
      ...
      ServiceYeuCauFile().delete(fileItem.id).then((value) {
