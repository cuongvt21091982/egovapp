...
class AppBarDangKyVPPXuLyAction extends StatefulWidget {
  const AppBarDangKyVPPXuLyAction({Key? key, required this.id,
    required this.title,
  ...
  final String title;
  @override
  _AppBarDangKyVPPXuLyActionState createState() => _AppBarDangKyVPPXuLyActionState();
}

class _AppBarDangKyVPPXuLyActionState extends State<AppBarDangKyVPPXuLyAction> {
  ...
  dynamic _delete() async
  {
    ...
    ServiceDangKy().delete(this.widget.id).then((value){
            ...
            PopupMenuButton(
                ...
                onSelected: (String value) async{
                  if(value==ParamUtils.actionUpdate) {
                    Navigator.push(context, new MaterialPageRoute(builder: (context) => new EditDangKy(id: this.widget.id,
                        title: Language.getText('capnhatdangkythietbi'), callBackRefresh: () =>
                            setState((){ })
                    )),);
                  }
