...
class ChuyenTrinhVanBanDi extends StatefulWidget
{
  const ChuyenTrinhVanBanDi({Key? key,
    required this.id,
    required this.title,
  }) : super(key: key);
  final String title;
  final int id;
  @override
  _ChuyenTrinhVanBanDiState createState() => _ChuyenTrinhVanBanDiState();}
class _ChuyenTrinhVanBanDiState extends State<ChuyenTrinhVanBanDi>
{
  ...
  Future<void> actionForm() async
  {
    ...
    ServiceToTrinh().add(
      ...
      ServiceVanBanDiFile().chuyenTrinh(value.id, this.widget.id, UserAuthSession.staffId).then((value) {
      }).catchError((e) {
