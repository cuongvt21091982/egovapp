...
import 'package:egovapp/service/meeting/service-meetingcomment.dart';
import 'package:flutter/material.dart';
import 'package:infinite_scroll_pagination/infinite_scroll_pagination.dart';
class MeetingYKienTongHopDialog extends StatefulWidget
{
  const  MeetingYKienTongHopDialog({Key? key, required this.id, required this.title}) : super(key: key);
  final int id;
  final String title;
  @override
  _MeetingYKienTongHopDialogState createState() => _MeetingYKienTongHopDialogState();
}
class _MeetingYKienTongHopDialogState extends State<MeetingYKienTongHopDialog> {

  String title = Language.getText('thongtinxuly');
  int page = Enviroments.currentPage;
  int size= Enviroments.pageSize;
  final PagingController<int,MeetingComment> _pagingController = PagingController(firstPageKey: 0);
  Future<void> _fetchPage(int pageKey) async {
    ServiceMeetingComment().getPaging(this.widget.id, UserAuthSession.staffId, page, size).then((value)
    {
      try {
  ...
  Widget build(BuildContext context) {
    ...
    var contentPage=PagedListView<int, MeetingComment>(
        pagingController: _pagingController,
        builderDelegate: PagedChildBuilderDelegate<MeetingComment>(
            ...
            itemBuilder: (context, item, index) =>  ListTile(
                  ...
                  children: [
                    Row(
                        ...
                        children: <Widget>[
                          ...
                          Spacer(),
                          if(item.meetingFileXuLys.length >0)
                ...
                onTap: (){
                  ...
                  for(var f in item.meetingFileXuLys)
                  {
