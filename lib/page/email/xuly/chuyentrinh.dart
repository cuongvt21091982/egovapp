...
class ChuyenTrinhThu extends StatefulWidget
{
  const ChuyenTrinhThu({Key? key,
    required this.id,
    required this.title,
  }) : super(key: key);
  final String title;
  final int id;
  @override
  _ChuyenTrinhThuState createState() => _ChuyenTrinhThuState();}
class _ChuyenTrinhThuState extends State<ChuyenTrinhThu>
{
  ...
  Future<void> actionForm() async
  {
    ...
    ServiceToTrinh().add(
      ...
      ServiceThu().chuyenTrinh(value.id, this.widget.id, UserAuthSession.staffId).then((value) {
                    ...
                    children: [
                      TextButton.icon(onPressed: () async {
                        Navigator.push(
                            context,
                            MaterialPageRoute<void>(
                              builder: (BuildContext context) => ThuNoiDungTrinh(id: this.widget.id, title: Language.getText("noidungtrinh")),
                              fullscreenDialog: true,
                            ));
                      },
