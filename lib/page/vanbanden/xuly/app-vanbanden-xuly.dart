...
import 'package:egovapp/page/vanbanden/xuly/assign-next.dart';
import 'package:egovapp/page/vanbanden/xuly/change-date.dart';
import 'package:egovapp/page/vanbanden/xuly/chenhoso.dart';
import 'package:egovapp/page/vanbanden/xuly/chuyentrinh.dart';
import 'package:egovapp/page/vanbanden/xuly/vanbanden-edit.dart';
import 'package:egovapp/page/vanbanden/xuly/ykientonghop.dart';
...
class AppBarVanBanDenXuLyAction extends StatefulWidget {
  const AppBarVanBanDenXuLyAction({Key? key, required this.id,
    required this.title,
  ...
  final String title;
  @override
  _AppBarVanBanDenXuLyAction createState() => _AppBarVanBanDenXuLyAction();
}

class _AppBarVanBanDenXuLyAction extends State<AppBarVanBanDenXuLyAction> {
  ...
  void initState() {
    ...
    ServiceVanBanDen().getById(this.widget.id).then((value){
      ...
      ServiceVanBanDenXuLy().getByMaVBAndMaXL(widget.id, UserAuthSession.staffId).then((value2) {
  ...
  dynamic _delete() async
  {
    ...
    ServiceVanBanDen().delete(this.widget.id).then((value){
  ...
  dynamic _changeStatus() async
  {
    final dialogContextCompleter = UIUtils.progressAction(context);
    final dialogContext = await dialogContextCompleter.future;
    ServiceVanBanDenXuLy().changeStatus(this.widget.id, ParamUtils.statusHoanThanh, UserAuthSession.staffId).then((value){
  ...
  dynamic _xuLyLai() async
  {
    final dialogContextCompleter = UIUtils.progressAction(context);
    final dialogContext = await dialogContextCompleter.future;
    ServiceVanBanDenXuLy().xuLyLai(this.widget.id, ParamUtils.statusChuaXuLy, UserAuthSession.staffId).then((value){
            ...
            PopupMenuButton(
                ...
                onSelected: (String value) async{
                  if(value==ParamUtils.actionUpdate) {
                    Navigator.push(context, new MaterialPageRoute(builder: (context) => new EditVanBanDen(id: this.widget.id,
                        title: Language.getText('capnhatvanbanden'), callBackRefresh: () =>
                            setState((){ })
                    )),);
                  }
                  ...
                  if(value==ParamUtils.actionChangeDate) {
                    new ChangeDateVanBanDen(id: this.widget.id, context: context, callBackRefresh: this.widget.callBackRefresh).showChange();
                  }
                  if(value==ParamUtils.actionAssign) {
                    Navigator.push(context, new MaterialPageRoute(builder: (context) => new AssignNextVanBanDen(id: this.widget.id,
                  ...
                  if(value==ParamUtils.actionSumComment) {
                    Navigator.push(
                        context,
                        MaterialPageRoute<void>(
                          builder: (BuildContext context) => VanBanDenYKienTongHopDialog(id: this.widget.id, title: this.widget.title),
                          fullscreenDialog: true,
                        ));
                  }
                  ...
                  if(value==ParamUtils.actionChuyenTrinh) {
                    Navigator.push(
                        context,
                        MaterialPageRoute<void>(
                          builder: (BuildContext context) => ChuyenTrinhVanBanDen(id: this.widget.id, title: Language.getText("chuyentrinh")),
                          fullscreenDialog: true,
                        ));
                  }
                  if(value==ParamUtils.actionChenHoSo) {
                    Navigator.push(
                        context,
                        MaterialPageRoute<void>(
                          builder: (BuildContext context) => VanBanDenChenHoSo(id: this.widget.id),
                          fullscreenDialog: true,
                        ));
                  }
                })
