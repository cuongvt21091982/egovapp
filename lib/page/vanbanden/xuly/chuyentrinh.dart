...
class ChuyenTrinhVanBanDen extends StatefulWidget
{
  const ChuyenTrinhVanBanDen({Key? key,
    required this.id,
    required this.title,
  }) : super(key: key);
  final String title;
  final int id;
  @override
  _ChuyenTrinhVanBanDenState createState() => _ChuyenTrinhVanBanDenState();}
class _ChuyenTrinhVanBanDenState extends State<ChuyenTrinhVanBanDen>
{
  ...
  Future<void> actionForm() async
  {
    ...
    ServiceToTrinh().add(
      ...
      ServiceVanBanDenFile().chuyenTrinh(value.id, this.widget.id, UserAuthSession.staffId).then((value) {
      }).catchError((e) {
