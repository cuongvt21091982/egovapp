...
import 'package:egovapp/model/vanbanden/vanbandencomment.dart';
import 'package:egovapp/page/files/download-file.dart';
import 'package:egovapp/page/files/filelink.dart';
import 'package:egovapp/service/vanbanden/service-vanbandencomment.dart';
import 'package:flutter/material.dart';
import 'package:infinite_scroll_pagination/infinite_scroll_pagination.dart';
class VanBanDenYKienTongHopDialog extends StatefulWidget
{
  const  VanBanDenYKienTongHopDialog({Key? key, required this.id, required this.title}) : super(key: key);
  final int id;
  final String title;
  @override
  _VanBanDenYKienTongHopDialogState createState() => _VanBanDenYKienTongHopDialogState();
}
class _VanBanDenYKienTongHopDialogState extends State<VanBanDenYKienTongHopDialog> {

  String title = Language.getText('thongtinxuly');
  int page = Enviroments.currentPage;
  int size= Enviroments.pageSize;
  final PagingController<int,VanBanDenComment> _pagingController = PagingController(firstPageKey: 0);
  Future<void> _fetchPage(int pageKey) async {
    ServiceVanBanDenComment().getPaging(this.widget.id, UserAuthSession.staffId, page, size).then((value)
    {
      try {
  ...
  Widget build(BuildContext context) {
    ...
    var contentPage=PagedListView<int, VanBanDenComment>(
        pagingController: _pagingController,
        builderDelegate: PagedChildBuilderDelegate<VanBanDenComment>(
            ...
            itemBuilder: (context, item, index) =>  ListTile(
                  ...
                  children: [
                    Row(
                        ...
                        children: <Widget>[
                          ...
                          Spacer(),
                          if(item.vanBanDenXLFiles.length >0)
                ...
                onTap: (){
                  ...
                  for(var f in item.vanBanDenXLFiles)
                  {
