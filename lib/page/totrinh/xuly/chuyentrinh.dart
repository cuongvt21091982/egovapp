...
class ChuyenTrinhToTrinh extends StatefulWidget
{
  const ChuyenTrinhToTrinh({Key? key,
    required this.id,
    required this.title,
  }) : super(key: key);
  final String title;
  final int id;
  @override
  _ChuyenTrinhToTrinhState createState() => _ChuyenTrinhToTrinhState();}
class _ChuyenTrinhToTrinhState extends State<ChuyenTrinhToTrinh>
{
  ...
  Future<void> actionForm() async
  {
    ...
    final valueContent = await controller.getText();
      ServiceToTrinh().add(
        ...
        ServiceToTrinh().chuyenTrinh(value.id, this.widget.id, UserAuthSession.staffId).then((value) {
                    ...
                    children: [
                      TextButton.icon(onPressed: () async {
                        Navigator.push(
                            context,
                            MaterialPageRoute<void>(
                              builder: (BuildContext context) => ToTrinhNoiDungTrinh(id: this.widget.id, title: Language.getText("noidungtrinh")),
                              fullscreenDialog: true,
                            ));
                      },
