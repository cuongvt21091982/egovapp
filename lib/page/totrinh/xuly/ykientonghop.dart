...
import 'package:egovapp/service/totrinh/service-totrinhcomment.dart';
import 'package:flutter/material.dart';
import 'package:infinite_scroll_pagination/infinite_scroll_pagination.dart';
class ToTrinhYKienTongHopDialog extends StatefulWidget
{
  const  ToTrinhYKienTongHopDialog({Key? key, required this.id, required this.title}) : super(key: key);
  final int id;
  final String title;
  @override
  _ToTrinhYKienTongHopDialog createState() => _ToTrinhYKienTongHopDialog();
}
class _ToTrinhYKienTongHopDialog extends State<ToTrinhYKienTongHopDialog> {

  String title = Language.getText('thongtinxuly');
  int page = Enviroments.currentPage;
  int size= Enviroments.pageSize;
  final PagingController<int,ToTrinhComment> _pagingController = PagingController(firstPageKey: 0);
  Future<void> _fetchPage(int pageKey) async {
    ServiceToTrinhComment().getPaging(this.widget.id, page, size).then((value)
    {
      try {
  ...
  Widget build(BuildContext context) {
    ...
    var contentPage=PagedListView<int, ToTrinhComment>(
        pagingController: _pagingController,
        builderDelegate: PagedChildBuilderDelegate<ToTrinhComment>(
            ...
            itemBuilder: (context, item, index) =>  ListTile(
                 ...
                 children: [
                   Row(
                       ...
                       children: <Widget>[
                         ...
                         Spacer(),
                         if(item.toTrinhFileNXLs.length >0)
              ...
              isThreeLine: true,
                onTap: (){
                        ...
                        for(var f in item.toTrinhFileNXLs)
                        {
