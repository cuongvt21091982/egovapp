...
class AssignToTrinh extends StatefulWidget
{
  const AssignToTrinh({Key? key,
    required this.id,
    required this.title,
  ...
  final int id;
  @override
  _AssignToTrinhState createState() => _AssignToTrinhState();}
class _AssignToTrinhState extends State<AssignToTrinh>
{
  ...
  Future<void> actionForm() async
  {
    ...
    ServiceToTrinh().joinToTrinh(this.widget.id, this.staffsSelect.map((e) => e.id).toList().join(',')).then((value) {
