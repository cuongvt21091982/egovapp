...
import 'package:egovapp/page/calendar/donvi/ykien.dart';
import 'package:egovapp/page/calendar/donvi/ykientonghop.dart';
import 'package:egovapp/page/calendar/xuly/bottom-bar-calendar-donvi-action.dart';
...
class LichDonVi extends StatefulWidget {
  const LichDonVi({Key? key}) : super(key: key);
  @override
  _LichDonViState createState() => _LichDonViState();
}
class _LichDonViState extends State<LichDonVi> {
  ...
  String donViList= ParamUtils.valueDefault.toString();
  ...
  dynamic yKienTongHop() async
  {
    Navigator.push(
        context,
        MaterialPageRoute<void>(
          builder: (BuildContext context) => CalendarDepartYKienTongHopDialog(
  ...
  final PagingController<int,CalendarDepartWeek> _pagingController = PagingController(firstPageKey: 0);
  Future<void> _fetchPage(int pageKey) async {
    ServiceCalendarDepart().getDataByWeekYear(currentWeek,
        this.yearCurrent,
        donViList,
        this.status,
        this.suDung,
        UserAuthSession.unitId).then((value)
    {
      try {
  ...
  void _createCalendar() {
    ...
    Navigator.push(context, new MaterialPageRoute(builder: (context) => new EditLichDonVi(id: 0, title: Language.getText('create_calendar'), callBackRefresh: () =>
  ...
  void showFile(List<CalendarDepartFile> value)
  {
  ...
  void showDetail(int id)
  {
    ...
    new LichDonViDetail(id: id, chuDe: Language.getText("thongtinlichdonvi"), callBackRefresh: () =>
        setState((){  })
    )));
  }
  void share()
  ...
  {
    List<EventCalendar> eventCalendarList=[];
    ServiceCalendarDepart().findByMonth(this.calendarMonthCurrent, this.calendarYearCurrent, UserAuthSession.unitId, status, suDung).then((value) {
      for(CalendarDepart item in value)
      {
  ...
  Widget build(BuildContext context) {
    ...
    var pageDS=PagedListView<int, CalendarDepartWeek>(
        pagingController: _pagingController,
        builderDelegate: PagedChildBuilderDelegate<CalendarDepartWeek>(
                                ...
                                return ListTile( minLeadingWidth: 0,
                                    ...
                                    title: Column(
                                      children: [
                                        ...
                                        if(item.calendars[index].calendarDepartFiles.length>0)
                                          Container(
                                              alignment: Alignment.topLeft,
                                              child:TextButton.icon(onPressed: () async {
                                                showFile(item.calendars[index].calendarDepartFiles);
                                              },
                                                  style:UIUtils.setNoButtonAttachViewStyle(),
                                                  icon: UIUtils.setNoButtonIcon(Icons.attach_file),
                                                  label: UIUtils.setNoButtonText(item.calendars[index].calendarDepartFiles.length.toString()+" "+ Language.getText("filedinhkem")
                                                  )
                                              )
                                          )
                                      ],
    ...
    return new WillPopScope(child:  Scaffold(
        appBar: AppBar(
          ...
          title: TextField(
            ...
            decoration: InputDecoration(
                ...
                hintText: Language.getText("lichdonvi").toUpperCase(),
        ...
        ), bottomNavigationBar: CalendarDepartBottomBarAction(
