...
class EditLichDonVi extends StatefulWidget
{
  const EditLichDonVi({Key? key,
    required this.id,
    required this.title,
  ...
  final int id;
  @override
  _EditLichDonViState createState() => _EditLichDonViState();}
class _EditLichDonViState extends State<EditLichDonVi>
{
  ...
  void initState(){
    ...
    if(this.widget.id !=0) {
      ...
      ServiceCalendarDepart().getById(this.widget.id).then((value) {
        ...
        if (value.calendarDepartFiles.length > 0) {
          for (var f in value.calendarDepartFiles)
  ...
  async {
    ...
    else
    {
      ...
      ServiceCalendarDepartFile().delete(fileItem.id).then((value) {
  ...
  Future<void> actionForm() async
  {
      ...
      ServiceCalendarDepart().add(
        ...
        for (var f in fileUploads) {
          if (f.path != null && f.action == false) {
            ServiceCalendarDepartFile().add(value.id, f.path!);
          }
        }
        Navigator.pop(dialogContext, true);
    ...
    {
      ServiceCalendarDepart().update(this.widget.id,
          FormatUtils.formatDateVN(dtTuNgay.text),
          dtTuGio.text,
          FormatUtils.formatDateVN(dtDenNgay.text),
          dtDenGio.text,
                    ...
                    Container(
                        ...
                        TextButton.icon(onPressed: () async {
                          if (formKey.currentState!.validate()) {
                            UIAction(actionForm).showDialogConfirm(context, Language.getText('message_save_lichdonvi_question'));
                          }
                        },
