...
import 'package:egovapp/page/calendar/canhan/detail.dart';
...
import 'package:egovapp/page/calendar/xuly/bottom-bar-calendar-canhan-action.dart';
...
class LichCaNhan extends StatefulWidget {
  const LichCaNhan({Key? key}) : super(key: key);
  @override
  _LichCaNhanState createState() => _LichCaNhanState();
}
class _LichCaNhanState extends State<LichCaNhan> {
  ...
  final PagingController<int,CalendarWeek> _pagingController = PagingController(firstPageKey: 0);
  Future<void> _fetchPage(int pageKey) async {
    ServiceCalendar().getDataByWeekYear(currentWeek,
        this.yearCurrent,
        UserAuthSession.staffId,
        this.status,
        this.suDung).then((value)
    {
      try {
  ...
  void _createCalendar() {
    ...
    Navigator.push(context, new MaterialPageRoute(builder: (context) => new EditLichCaNhan(id: 0, title: Language.getText('create_calendar'), callBackRefresh: () =>
  ...
  void showFile(List<CalendarFile> value)
  {
  ...
  void showDetail(int id)
  {
    ...
    new LichCaNhanDetail(id: id, chuDe: Language.getText("thongtinlichcanhan"), callBackRefresh: () =>
        setState((){  })
    )));
  }
  ...
  {
    List<EventCalendar> eventCalendarList=[];
    ServiceCalendar().findByMonth(this.calendarMonthCurrent, this.calendarYearCurrent, UserAuthSession.staffId, status, suDung).then((value) {
      for(Calendar item in value)
      {
  ...
  Widget build(BuildContext context) {
    ...
    var pageDS=PagedListView<int, CalendarWeek>(
        pagingController: _pagingController,
        builderDelegate: PagedChildBuilderDelegate<CalendarWeek>(
                                ...
                                return ListTile( minLeadingWidth: 0,
                                    ...
                                    title: Column(
                                      children: [
                                        ...
                                        if(item.calendars[index].calendarFiles.length>0)
                                          Container(
                                              alignment: Alignment.topLeft,
                                              child:TextButton.icon(onPressed: () async {
                                                showFile(item.calendars[index].calendarFiles);
                                              },
                                                  style:UIUtils.setNoButtonAttachViewStyle(),
                                                  icon: UIUtils.setNoButtonIcon(Icons.attach_file),
                                                  label: UIUtils.setNoButtonText(item.calendars[index].calendarFiles.length.toString()+" "+ Language.getText("filedinhkem")
                                                  )
                                              )
                                          )
                                      ],
    ...
    return new WillPopScope(child:  Scaffold(
        appBar: AppBar(
          ...
          title: TextField(
            ...
            decoration: InputDecoration(
                ...
                hintText: Language.getText("lichcanhan").toUpperCase(),
        ...
        ), bottomNavigationBar: CalendarCaNhanBottomBarAction(
