...
class EditLichCaNhan extends StatefulWidget
{
  const EditLichCaNhan({Key? key,
    required this.id,
    required this.title,
  ...
  final int id;
  @override
  _EditLichCaNhanState createState() => _EditLichCaNhanState();}
class _EditLichCaNhanState extends State<EditLichCaNhan>
{
  ...
  void initState(){
    ...
    if(this.widget.id !=0) {
      ...
      ServiceCalendar().getById(this.widget.id).then((value) {
        ...
        if (value.calendarFiles.length > 0) {
          for (var f in value.calendarFiles)
  ...
  async {
    ...
    else
    {
      ...
      ServiceCalendarFile().delete(fileItem.id).then((value) {
  ...
  Future<void> actionForm() async
  {
      ...
      ServiceCalendar().add(
        ...
        for (var f in fileUploads) {
          if (f.path != null && f.action == false) {
            ServiceCalendarFile().add(value.id, f.path!);
          }
        }
        Navigator.pop(dialogContext, true);
    ...
    {
      ServiceCalendar().update(this.widget.id,
          FormatUtils.formatDateVN(dtTuNgay.text),
          dtTuGio.text,
          FormatUtils.formatDateVN(dtDenNgay.text),
          dtDenGio.text,
                    ...
                    Container(
                        ...
                        TextButton.icon(onPressed: () async {
                          if (formKey.currentState!.validate()) {
                            UIAction(actionForm).showDialogConfirm(context, Language.getText('message_save_canhan_question'));
                          }
                        },
