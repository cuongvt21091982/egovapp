...
final String apiUrl = ApiUtils().apiUrl+'/api/info/vanbandenxl';
class ServiceVanBanDenXuLy
{
  Future<List<VanBanDenXuLy>> getPaging(String keyword, String status, int phoCap, int maNguoiXuLy,
  int nguoiChuTriId, int page, int size) async {
    ...
    return responseJson.map((e) => VanBanDenXuLy.fromJson(e)).toList();
  ...
  Future<VanBanDenXuLy> getByMaVBAndMaXL(int id, int staffId) async {
    final response = await http.get(
  ...
  Future<List<VanBanDenXuLy>> getAllByMaVB(int id) async {
    final response = await http.get(
